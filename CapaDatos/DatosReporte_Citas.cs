        ...
        public static List<EntidadReporte_Citas> ListarReporteCitas( string fecha1, string fecha2)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conexion"].ToString()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(strSProcedure, con))
                {
                    ...
                    while (dr.Read())
                    {
                        EntidadReporte_Citas objentidad = new EntidadReporte_Citas()
                        {
